...
function setGiveawayMessageId(giveawayId, messageId) {
...
function toggleGiveawayEntry(giveawayId, userId) {
    ...
    let entered = true;
    if (entries.has(userId)) { entries.delete(userId); entered = false; }
    else { entries.add(userId); }
    db.prepare('UPDATE giveaways SET entries = ? WHERE id = ?').run(JSON.stringify([...entries]), giveawayId);
    return entered;
}
function endGiveaway(giveawayId, winners) {
    db.prepare('UPDATE giveaways SET ended = 1, winners = ? WHERE id = ?').run(JSON.stringify(winners), giveawayId);
}
