...
const { ROLE_BADGE_STYLE_CHOICES, dispatchAutocomplete, autocompleteColor, autocompleteRoleBadgeStyle } = require('../utils/autocomplete.js');
...
async function renderRoleBadge({ text, style, primary, secondary, font }) {
    ...
    if (style === 'pill') {
        ...
        ctx.beginPath();
        ctx.moveTo(W/2, 2); ctx.lineTo(W-2, H/2); ctx.lineTo(W/2, H-2); ctx.lineTo(2, H/2);
        ctx.closePath(); ctx.fill();
    }
...
module.exports = {
    ...
    async autocomplete(interaction) {
        await dispatchAutocomplete(interaction, { style: autocompleteRoleBadgeStyle, primary_color: autocompleteColor, secondary_color: autocompleteColor });
    },
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(primary)
            .addFields({ name: 'Style', value: styleLabel, inline: true }, { name: 'Font', value: font, inline: true })
